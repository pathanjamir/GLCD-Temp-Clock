...
void i2cByteWrite(u8 slaveAddr8bit,u8 wBuffAddr,u8 dat)
{
  i2cStart();
  i2cWrite(slaveAddr8bit&0xfe);//ensuring upper 7bits intact & lsb(r/w)  bit is 0
  i2cAck();
  i2cWrite(wBuffAddr);//address of buff for writing data into
  i2cAck();
  i2cWrite(dat);//data for writing to that address location
  i2cAck();
  i2cStop();
  delay_ms(10);  
}

u8 i2cRandomRead(u8 slaveAddr8bit,u8 rBuffAddr)
{
  u8 buff;
  i2cStart();
  i2cWrite(slaveAddr8bit&0xfe);//Slave addr + w
  i2cAck();
  i2cWrite(rBuffAddr);
  i2cAck();
  i2cReStart();
  i2cWrite(slaveAddr8bit|0x01); //Slave addr + r
  i2cAck();
  buff=i2cRead();
  i2cNAck();
  i2cStop();
